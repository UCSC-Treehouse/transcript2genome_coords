...
gtf <-  gtf_raw %>%
  filter(feature == "exon") %>%
  rename(open_end = end) %>%
  mutate(end = open_end +1) %>% # I was assuming half-open, but it was  end-inclusive
...
conversion_results_raw <- left_join(transcript_coord_input, 
          gtf_relevant_with_pos_list,
           by = "transcript_id") %>%
    rowwise() %>%
  mutate(genome_position_of_modification = genome_pos[mod_position])

conversion_results <- conversion_results_raw %>%
  select(gene_id, transcript_id, transcript_position = mod_position,
